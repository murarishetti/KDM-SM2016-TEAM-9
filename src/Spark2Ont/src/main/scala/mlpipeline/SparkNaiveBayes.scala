...
object SparkNaiveBayes {
  ...
  def main(args: Array[String]) {
    ...
    val CATEGORIES = List("Design", "Features")
    var index = 0
  ...
  private def preprocess(sc: SparkContext,
    ...
    val df = sc.wholeTextFiles(paths).map(f => {
      var ff = f._2
     // ff.foreach(m => CoreNLP.returnNER(m))

      ff.split(" ").foreach(m => (map(m) = CoreNLP.returnNER(m)))
      val owl = new OwlPizza()
      owl.createClass(":Contract")
      var i = 0
      map.foreach(k=> {
        if(k._2.toLowerCase.contains("location")) {
          owl.createIndividual(":" + k._1, ":Country")
          owl.createObjectProperty(":" + ff, "hasTopic", ":Topic" + i)
        }
        else if(k._2.toLowerCase().contains("money")) {
          owl.createIndividual(":" + k._1,":Price")
          owl.createObjectProperty(":" + ff, "hasTopic", ":Topic" + i)
        }
        else if(k._1.toLowerCase().contains("gb")) {
          owl.createIndividual(":" + k._1,":Memory")
          owl.createObjectProperty(":" + ff, "hasTopic", ":Topic" + i)
        }
        else if(k._2.toLowerCase().contains("date")) {
          owl.createIndividual(":" + k._1,":Date")
          owl.createObjectProperty(":" + ff, "hasTopic", ":Topic" + i)
        }
        else if(k._2.toLowerCase().contains("mp")) {
          owl.createIndividual(":" + k._1,":Camera")
          owl.createObjectProperty(":" + ff, "hasTopic", ":Topic" + i)
        }
        else if(k._2.toLowerCase().contains("duration")) {
          owl.createIndividual(":" + k._1,":Contract")
          owl.createObjectProperty(":" + ff, "hasTopic", ":Topic" + i)
        }
      } )
      //print(map)
